...
int main()
{
	...
	// binding the socket to the server system
    ...
    { 
        ...
        //sockfd is a socket that is created and binded using bind, and is listening for connections (here, server socket)
        new_sockfd = accept(sockfd, (struct sockaddr *)&cli, &len);
        if (new_sockfd == -1)
            printf("accepting connection");
        printf("server: got connection from %s port %d\n",
               inet_ntoa(cli.sin_addr), ntohs(cli.sin_port)); //address and port transmitted in network-byte order.
               //these functions convert these network-bytes into human-readable text
        char temp_key[50];
        char cur_name[50];
